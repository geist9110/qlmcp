...
public class OAuth2TokenController {
    ...
    public ResponseEntity<?> token(
        ...
        try {
            // 클라이언트 인증
            ClientCredentials credentials = extractClientCredentials(authHeader, clientId,
                clientSecret);
    ...
    private ClientCredentials extractClientCredentials(
        ...
        if (authHeader != null && authHeader.startsWith("Basic ")) {
            ...
            return new ClientCredentials(parts[0], parts.length > 1 ? parts[1] : null);
        }
    ...
    private RegisteredClient authenticateClient(ClientCredentials credentials) {
