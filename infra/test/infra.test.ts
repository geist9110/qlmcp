  ...
  describe("MCP 서버 테스트", () => {
    test("인스턴스 타입 테스트", () => {
      template.hasResourceProperties("AWS::EC2::Instance", {
        Tags: Match.arrayWith([{ Key: "Name", Value: "qlmcp-mcp-server" }]),
    ...
    test("Role이 인스턴스에 연결되는지 테스트", () => {
      ...
      const [instanceId, instance] = Object.entries(resources).find(
        ([_, res]: any) =>
          res.Type === "AWS::EC2::Instance" &&
          res.Properties.Tags?.some(
            (t: any) => t.Key === "Name" && t.Value === "qlmcp-mcp-server",
          ),
      ) as [string, any];
  ...
  describe("Security Group 테스트", () => {
    describe("메인 서버 Security Group 테스트", () => {
      test("Ingress 규칙이 3개인지 테스트", () => {
        ...
        const ingressRules =
          securityGroups[sgKey].Properties.SecurityGroupIngress;
